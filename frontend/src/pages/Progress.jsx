...
export default function Progress() {
  ...
  /*
   * IMPORTANT:
   * videos[i] is the athlete's own video number.
   *
   * Database ID can be 11, 14, 20...
   * But UI will show 1, 2, 3...
   */
  const getDisplayVideoNumber = (videoId) => {
    ...
    return index >= 0 ? index + 1 : videoId;
  };

  const withQuality =
    points?.filter((p) => p.quality_score !== null) || [];

  const withRisk =
    points?.filter((p) => p.risk_score !== null) || [];
  ...
  const trend = (() => {
    ...
    const secondAvg = avg(
      secondHalf.length ? secondHalf : firstHalf
    );
        ...
        {!loading &&
          ...
          points.length >= 2 && (
            <>
              {trend && (
                <div
                  className={`trend-banner fade-in-up trend-${trend.direction}`}
                  style={{ '--delay': '0.05s' }}
                >
                  ...
                  <span>
                    ...
                    {trend.diff > 0 ? '+' : ''}
                    {trend.diff.toFixed(1)} points on average across your recent sessions.
                  </span>
                </div>
              )}

              <div
                className="role-panel fade-in-up stagger"
                style={{ '--delay': '0.1s' }}
              >
                <h3>
                  Quality Score &amp; Risk Score Over Time
                </h3>
                ...
                <div className="chart-legend">
                  <span className="legend-item">
                    <span className="legend-dot quality" />
                    Quality Score
                  </span>
                  ...
                  <tbody>
                    {points.map((p) => (
                      <tr key={p.video_id}>
                        ...
                        {/* DISPLAY NUMBER ONLY */}
                        <td>
                          #{getDisplayVideoNumber(p.video_id)}
                        </td>
                        ...
                        <td>
                          {p.risk_category ? (
                            <span
                              className={`risk-pill ${p.risk_category}`}
                            >
                              {p.risk_category}
                            </span>
                          ) : (
...
function ProgressChart({ points }) {
  ...
  const validQuality = points.filter(
    (p) => p.quality_score !== null
  );

  const validRisk = points.filter(
    (p) => p.risk_score !== null
  );
  ...
  return (
    ...
    >
      ...
      <path
        ...
        className="chart-line quality-line"
      />
      ...
      <path
        ...
        className="chart-line risk-line"
      />
      ...
      {validQuality.map((p, i) => (
        <circle
          ...
          cy={yFor(p.quality_score)}
          r="4"
          className="chart-dot quality-dot"
        />
      ))}
      ...
      {validRisk.map((p, i) => (
        <circle
          key={`r-${i}`}
          cx={xFor(points.indexOf(p))}
          cy={yFor(p.risk_score)}
          r="4"
          className="chart-dot risk-dot"
        />
      ))}
    </svg>
  );
